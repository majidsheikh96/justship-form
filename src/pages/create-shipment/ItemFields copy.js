        ...
        <div>
                            ...
                            <div className="flex flex-col my-2" key={itemIndex}>
                                <div className="rounded-xl group border border-border-dark-gray shadow-border grid grid-cols-[38px_auto_18%_15%_15.5%] gap-x-1.5 relative h-[36px] w-full">
                                    ...
                                    <input
                                        ...
                                        inputMode="numeric"
                                        id="quantity-p0i0"
                                        className={`focus:outline-none px-1 border-2 border-transparent placeholder:text-placeholder-gray ring-1 focus:ring-light-purple rounded-xl  ring-error-red ${errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.quantity && 'ring-rose-500'}`}
                                        placeholder="8"
                                        {...register(`package.${packageIndex}.box.items.${itemIndex}.quantity`, { required: "Required", min: 1 })}
                                    />
                                    <div className={`ring-error-red flex items-center relative ring-[1.5px] focus-within:ring-light-purple rounded-xl group ${errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.description && 'ring-rose-500'}`}>
                                        ...
                                        <input
                                            ...
                                            {...register(`package.${packageIndex}.box.items.${itemIndex}.description`, { required: "Required", maxLength: 30, minLength: 4 })}
                                        />
                                    </div>
                                    <div className={`flex flex-wrap items-center rounded-xl ring-[1.5px] focus-within:ring-light-purple relative ${errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.category && 'ring-rose-500'}`}>
                                        <select
                                            id="category-p0i0"
                                            className={`focus:outline-none appearance-none bg-transparent w-full h-full rounded-xl mx-0.5 text-placeholder-gray`}
                                            {...register(`package.${packageIndex}.box.items.${itemIndex}.category`, { required: "Required" })}
                                        >
                                            <option
                                                value="none"
                                    ...
                                    <div className={`px-1 flex items-center border-transparent ring-[1.5px] focus-within:ring-light-purple rounded-xl  ring-error-red ${errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.totalValue && 'ring-rose-500'}`}>
                                        ...
                                        <input
                                            ...
                                            placeholder="60"
                                            {...register(`package.${packageIndex}.box.items.${itemIndex}.totalValue`, { required: "Required", min: 1 })}
                                        />
                                    </div>
                                    <label htmlFor="total weight" hidden=" ">
                                        item total weight
                                    </label>
                                    <div className={`flex items-center border-transparent justify-end pr-2 ring-[1.5px] focus-within:ring-light-purple rounded-xl  ring-error-red ${errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.totalWeight && 'ring-rose-500'}`}>
                                        <input
                                            type="number"
                                            inputMode="decimal"
                                            ...
                                            placeholder="4.0"
                                            {...register(`package.${packageIndex}.box.items.${itemIndex}.totalWeight`, { required: "Required", min: 1 })}
                                        />
                                        <p className="text-dark-purple">kg</p>
                                    </div>

                                    {itemFields.length > 1 && <div onClick={() => itemRemove(itemIndex)} className="rounded-full group flex items-center my-auto border border-transparent bg-dark-purple hover:bg-light-purple w-[24px] h-[24px] hover:cursor-pointer absolute -right-2.5 -top-2.5 group-hover:block" title="Remove">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mx-auto w-3/4 stroke-white" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth="2">
                                ...
                                <div className="flex mb-6 pl-12 w-full select-none justify-between">
                                    ...
                                    {errors?.package?.[packageIndex]?.box?.items?.[itemIndex]?.description && <p className=" h-1 text-xs text-red-500">
            ...
            <div className="grid grid-cols-[78%_22%] items-center font-semibold px-8 pb-8">
                ...
                <p className="md:text-right text-dark-purple md:mr-5">
                    {
                        getTotalItemsValue(packageIndex)
                    }
                </p>
            </div>
        </div>
